...
import {
  ...
  findMassForDilationFactorAndRadius,
} from '../utils/physics';
...
export const GravitationalTimeDilationExperiment: React.FC = () => {
  ...
  const mass = controlMode === 'factor'
    ? findMassForDilationFactorAndRadius(dilationFactor, radius)
    : (massUnit === 'earth' ? massValue * EARTH_MASS : massValue * SOLAR_MASS);
      ...
      <div className="bg-gradient-to-br from-white/95 to-white/90 backdrop-blur-sm p-4 sm:p-6 rounded-xl shadow-2xl border border-white/20">
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-gray-500 mt-2">
            {controlMode === 'factor'
              ? 'Contrôlez le facteur de dilatation temporelle et le rayon, la masse s\'ajuste automatiquement'
          ...
          {controlMode === 'factor' && (
            <>
              ...
              <div className="bg-green-50 border border-green-200 p-3 rounded-lg">
                <div className="text-xs font-semibold text-green-700 mb-2">Masse Calculée Automatiquement</div>
                <div className="text-xl font-bold text-green-800">{(mass / SOLAR_MASS).toFixed(4)} M☉</div>
                <p className="text-xs text-green-600 mt-2">
                  Pour obtenir un facteur {dilationFactor.toFixed(2)}× avec un rayon de {formatRadius(radius)}, la masse requise est {(mass / SOLAR_MASS).toFixed(4)} masses solaires
                </p>
              </div>
            </>
          )}
